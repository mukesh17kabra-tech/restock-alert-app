...
5. Required scopes: `read_products,read_inventory,read_locations`
   (`read_locations` is required alongside `read_inventory` for the
   `inventory_levels/update` webhook to validate. Note: registering
   webhooks via the Admin API doesn't need its own scope — there's no
   such thing as a `write_webhooks` scope, despite how it sounds.)
